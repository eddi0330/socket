...
def genera_richieste1(num,address,port):
    ...
    studenti=['Myrdari', 'Cognome1', 'Cognome2', 'Cognome3', 'Cognome4']
...
def genera_richieste3(num,address,port):
  ...
  #                        .....}
    ...
    studenti=random.choice(['Myrdari', 'Cognome1', 'Cognome2', 'Cognome3', 'Cognome4'])
    materie=['Matematica', 'Italiano', 'Inglese', 'Storia', 'Geografia']
    i=0
    data={}
    for studente in studenti:
    ...
    if not data:
        ...
        for elemento in data:
            print(f"{threading.current_thread().name}: Lo studente {elemento} ha una media di {elemento['media']:.2f} e assenze: {elemento['assenze']:.2f}")
    s.close()
...
if __name__ == '__main__':
    ...
    while n<NUM_WORKERS:
        ...
        genera_richieste3(NUM_WORKERS,SERVER_ADDRESS,SERVER_PORT)
        n+=1
    end_time=time.time()
    ...
    for i in range(NUM_WORKERS):
        ...
        #t=Thread(target=genera_richieste2,args=(NUM_WORKERS,SERVER_ADDRESS,SERVER_PORT))
        t=Thread(target=genera_richieste3,args=(NUM_WORKERS,SERVER_ADDRESS,SERVER_PORT))
           #la i indica il numero delle volte che è già stato ripetuto il ciclo
        listaT.append(t)
    ...
    for i in range(NUM_WORKERS):
        process.append(multiprocessing.Process(target=genera_richieste3,args=(NUM_WORKERS,SERVER_ADDRESS,SERVER_PORT)))   #la i indica il numero delle volte che è già stato ripetuto il ciclo
